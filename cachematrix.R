...
#  to the example: makeVector().  makeCacheMatrix creates a list of functions 
#  to set/get the matrix and to set/get value of its inverse.  Only the different 
#  of makeCacheMatrix is that x's default value is an empty matrix while 
#  makeVector's x's default value is an empty numeric vector().
# 
#  Since the example makeVector in the assignement works for any data type, 
#  it didn't even need to be changed to be used by cacheSolve().
...
#  But the requirement is the requirement unless we are alloed to change it 
#  even when it is ambiguous and wrong.
# 
#  So here's the required short description of the function.
# 


#  makeCacheMatrix returns a list of functions to set/get the data and
